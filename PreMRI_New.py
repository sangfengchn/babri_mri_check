...
def fun_move(file_path, out_root, machine='OLD'):
    ...
    if machine == 'OLD':
        ...
        squences = {
            ...
            'bold_80x80_s2_2_5iso_tr2s_195': 'REST',
...
if __name__ == "__main__":
    ...
    for i in os.listdir(src):
        ...
        newDf.loc[index, 'CheckDateName'] = 'yes'
        ...
        res_check_series_a = ''
        status_list = newDf.loc[index, '结构']
        status_file = os.path.join(dst, 'T1', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'T1;'
        # for rest
        status_list = newDf.loc[index, '静息态']
        status_file = os.path.join(dst, 'REST', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'REST;'
        # for em
        status_list = newDf.loc[index, '情景记忆']
        status_file = os.path.join(dst, 'EM', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'EM;'
        # for wm
        status_list = newDf.loc[index, '工作记忆']
        status_file = os.path.join(dst, 'NBACK', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'NBACK;'
        # for dti
        status_list = newDf.loc[index, 'DTI']
        status_file = os.path.join(dst, 'DTI', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'DTI;'
        # for flair
        status_list = newDf.loc[index, 'flair']
        status_file = os.path.join(dst, 'FLAIR', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'FLAIR;'
        # for clinical
        status_list = newDf.loc[index, 'T2WI']
        status_file = os.path.join(dst, 'T2WI', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'T2;'
        # for LANT
        status_list = newDf.loc[index, '失独老人任务']
        status_file = os.path.join(dst, 'LANT', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'LANT;'
        # for TOF
        status_list = newDf.loc[index, 'TOF']
        status_file = os.path.join(dst, 'TOF', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'TOF;'
        # for ASL
        status_list = newDf.loc[index, 'ASL']
        status_file = os.path.join(dst, 'ASL', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'ASL;'
        
        # for SpatialTask
        status_list = newDf.loc[index, '空间导航任务']
        status_file = os.path.join(dst, 'SpatialTask', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'SpatialTask;'

        # for hippo
        status_list = newDf.loc[index, 'hippo']
        status_file = os.path.join(dst, 'HIPPO', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += 'HIPPO;'
        # for 3D-flair
        status_list = newDf.loc[index, '3D-flair']
        status_file = os.path.join(dst, '3D-FLAIR', f'{tar_name}.tar.gz')
        if (status_list == '√') and (not os.path.exists(status_file)):
            res_check_series = 'no'
            res_check_series_a += '3D-FLAIR;'
